...
phenoMean       = 6.81     # Bouffier et al., 2008
heritability    = 0.33    # Bouffier et al., 2008 (constante au cours des genrations)
CV              = 0.0505   # Bouffier et al., 2008
...
## LOOP FOR TESTING ACCURACY OF EBV 

accuracy.test = c(0.5,0.8,1)

for(acc in accuracy.test){
  ...
  if(T){
    ...
    data$type       = accuracy.BV.EBV
    assign(paste("data",accuracy.BV.EBV, sep="_"), data)
  }
...
DATA.ALL = NULL
for(acc in accuracy.test){
  dt = get(paste("data", acc, sep="_"))
  DATA.ALL = rbind(DATA.ALL, dt)
}
...
plot =DATA.ALL %>%
    ...
    geom_density_ridges(aes(fill = factor(type), color=factor(type)), alpha = .4, rel_min_height = 0.01,quantile_lines=TRUE, quantile_fun=function(x,...)mean(x)) +
    ylab("Generations")+xlab("values")

ggsave("EvolutionsBP_EBVacc.tiff", plot, width= 40, height= 20, units= "cm", scale = 1, dpi= 600)
